...
def work(url, city, language):
    if url:
        ...
        if resp.status_code == 200:  # review - потрясающе, если что не так, и url будет недоступен, то resp будет None
            # и мы попытаемся у None достать status_code
            ...
            if not new_jobs:
                ...
                if table:
                    tr_list = table.find_all('tr',
                                             attrs={'id': True})  # там id указаны как id="8360908" (любое другое число)
                    for tr in tr_list:
